...
def dispLetter(letter, position, hasDecimal):
    ...
    if hasDecimal:
        myArduino.digital_write(decimal, 1)
    else:
        myArduino.digital_write(decimal, 0)
...
def lightUp(word):
    ...
    if len(deciWord) > 4:
        decimalPos = deciWord.index(".")-1
    
    length = len(word)
    for i in range(length):
        if decimalPos == i:
            dispLetter(word[i], i, True)
        else:
            dispLetter(word[i], i, False)
